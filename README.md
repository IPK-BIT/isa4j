...
In most cases it makes sense to build up the Investigation File first and define all the Parameters, Protocols, Ontologies etc. that you want to use later in your Study and Assay Files.
With only a few exceptions, isa4J classes and class attributes are named in keeping with the [ISA model schemas](https://github.com/ISA-tools/isa-api/tree/master/isatools/resources/schemas/isa_model_version_1_0_schemas/core), so if you're familiar with those or the [python API](https://github.com/ISA-tools/isa-api), isa4J should feel quite intuitive to you.
If you're not, you can read this documentation and just explore for a bit until you get the general design concept. 
...
If something is still not clear after that, feel free to get in touch with us.
...
#### 3.1.2 Ontologies
If you want to refer to Ontologies lateron in the Investigation or any of the Study or Assay Files, define them here and add them to your Investigation.
...
In addition to a simple value, OntologyAnnotations allow you to connect that `term` to an Ontology(`term source`) and identify it with a `term acession number`.
